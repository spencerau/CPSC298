...
/* ?? add initializers for member variables m_iRowBoat, m_iColumnBoat, and m_nTorpedoes. ?? */
{
	m_nGridRows = k_nGridRowsDefault;
	...
	for (int iRow = 0; iRow < this->m_nGridRows; iRow++) {
		...
		for (int iColumn = 0; iColumn < this->m_nGridColumns; iColumn++) {
			if ('V' == this->a_cGrid[iRow][iColumn]) {
				if (bShowBoat) std::cout << "V" << "|";
				else std::cout << "~" << "|";
			}
			else std::cout << a_cGrid[iRow][iColumn] << "|";
		}
		std::cout << std::endl;
	}
	return;
